...
document's groups by name, derived from the definition the browser already
holds (23's verbatim rule is the browser's too). Picking a name creates one
instance of that group, landing where the menu was opened — where the user's
attention is, 12's drop-point precedent. With no groups in the document the
submenu is simply absent — a row that cannot act is noise, not a state to
surface. The submenu tracks the definition as it changes: packaging a
...
- With one or more groups defined in the document, the canvas background's
  context menu carries a Groups submenu listing them by name, derived from
  the definition the browser holds; picking a group creates one instance of
  it landing where the menu was opened, the server assigning the instance
  uuid and recording the position in the node's metadata; with no groups
  defined no Groups entry appears, and the submenu tracks the definition
  live — packaging a selection adds its group to the menu, unpacking a
  definition's last instance removes it. (REQ-44, REQ-45, REQ-41, REQ-12)
...
- Focused tests cover: create with a document group reference — uuid
  ...
  and pushed to every connection; create honouring the groups-before-listing
  resolution order where a held document transiently carries a name a linked
  plugin also provides; create with an unknown group reference refused with
  the definition untouched. UI tests (20's runner) cover the submenu derived
  from the held definition, the instance landing at the opened menu's
...
- 2026-09-16 — UX calls settled here: the background context menu rather
  ...
  document-shaped list wants a document-shaped surface; the submenu is
  absent rather than shown disabled when no groups exist, since a menu row
  that cannot act is noise — the palette's empty message is the different
