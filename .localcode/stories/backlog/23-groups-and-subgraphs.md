...
- **Subgraph representation in files: named groups in the same document.**
  ...
  1 and 2 — a version 1 file means exactly what it always meant — and writes
  version 2; round-tripping carries groups verbatim, inner metadata
  included.
- **Compile-time inlining vs nested execution: flatten at compile.**
  ...
  identities derived deterministically from their group instance's uuid,
  collisions across groups made harmless by the same namespacing; boundary
  edges are rewired so an exposed input receives the group instance's
...
- **What a running group exposes to the UI: the boundary, and an honest
  ...
  fail-fast ends the run, and the report names the group and the inner node,
  so an inside mistake is findable from the outside; completed when every
  inner node is completed; running otherwise. Inner value pulses stay
  inside — collapsed means collapsed, and unpacking is the way in (25).

The editor consumes groups without a new mechanism. A group instance renders
by the same default node class as any type: label defaulting to the group's
name and overridable, ports from the group's exposed ports with 19's type
channel, inline scalar fields on scalar-possible unconnected exposed inputs,
connected indicators when wired. The sidebar edits the instance like any
node (14), and the gestures already shipped work on it untouched: wiring
...
- A graph file can define groups: an optional `groups` section whose entries
  ...
  cycle. The reader still loads version 1 files exactly as before, writes
  version 2, and a load–dump round trip preserves groups verbatim — names,
  ports, bindings, inner nodes, inner edges, inner metadata. (REQ-44,
  REQ-11)
- The new shape fails loudly and precisely, each error naming the group and
  where it sits: a duplicate group name, an instance referencing a group the
  document does not define, an inner edge landing on a node outside its own
  group, a binding naming a node or port its group does not contain, a group
  name colliding with a linked plugin's type reference, a group-reference
  cycle, an exposed port whose declared types cannot bridge to the inner
  port it binds. Uuids must be unique within each graph; collisions across
  groups are legal, namespaced at compile. A group no instance references is
  not an error — dead weight is the hand-writer's to see, not a failure mode
  to invent. (REQ-71, REQ-72)
- Compiling a definition with group instances yields one flat compiled graph
  ...
  nodes with identities derived deterministically from their group instance,
  boundary edges rewired to the external world, an unconnected exposed input
...
- Events for nodes inside a group carry identities that name the group they
  sit in, so a terminal subscriber's event timeline stays readable and a
  grouped run is as inspectable as a flat one. (REQ-13)
- In the editor, a file with groups opens showing each group instance as one
  ...
  instance only. The palette lists linked plugin types only. No protocol
  message changes: the definition the browser holds carries the groups
  verbatim. (REQ-44, REQ-37, REQ-38, REQ-56, REQ-58, REQ-47)
- Running a grouped graph animates the group's boundary ports and wires like
  ...
  error when an inner node errors, the report naming the group and the inner
  node; completed when all inner nodes are; running otherwise — while edits
  to a group instance go quiet under the run lock with every other edit.
...
- Focused tests cover: the round trip with groups; each loader error path
  above; flattening — identities, boundary rewiring, a parameter flowing
...
- The visible proof, run per DEVELOPMENT.md: the editor example's sample
  ...
  ordinary ones, ports and wires as normal; wire a node to one of its
  exposed inputs, run, and see the boundary animate; break something inside
  a group in the file, reopen, run, and see the run stop with the error
  naming the group and the inner node; run the same file headless from a
...
- 2026-09-16 — Why flatten: the vision's one-of-each-mechanism line. Nested
  ...
  so events and errors reach the user labelled with the group and inner
  node they name — the readable part — rather than raw definition uuids.
- 2026-09-16 — Version handling: the reader's supported versions become
  ...
  always meant. This is the first use of the version field for growth, and
  it is additive only — the vision's migration stance (a version field is
  enough, no tooling) respected.
