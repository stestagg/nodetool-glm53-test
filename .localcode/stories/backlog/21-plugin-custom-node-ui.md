...
- **One mechanism, two attachment points.** A plugin declares UI through
  ...
  *node-type UI* — a bundle that replaces the default node rendering for
  ...
  display wherever values display (story 17's per-port readouts) and are
  handed ready-rendered to any node UI that wants them. Two points because
  they answer two different questions — "how does this node look and edit?"
  and "what does a value of this type look like on screen?" — but one
...
- **What custom node UI owns, and what it never takes.** The plugin's
  component owns everything between the ports: it receives the node's
  definition slice — label, parameters, connected inputs — and the live
  display state — the status marks and per-port latest values, serialised
  — and renders the node's content as it pleases. The ports stay
  editor-rendered, and with them every gesture: wiring (13), moving,
  selecting, deleting, panning all behave on a custom node exactly as on a
  default one, because the gestures land on structure the editor draws.
  This is the line that keeps the canvas coherent — a user should never
  have to relearn a gesture because a node looks different — and it is
  also what keeps the story small: the plugin buys content, not a second
  canvas. Editing from inside the plugin's UI rides the existing operation
  messages — label and parameter edits, the same ones the sidebar and
  inline fields issue — through small helpers the editor hands the
  component; there is no second editing path, and story 16's server-enforced
  lock rejects a plugin-issued edit while running identically to any other.
...
- **Contract version and failure, both visible.** The component contract
  ...
  load — missing asset, wrong contract version — is reported through
  story 18's surfaces and the node falls back to the default rendering:
  usable regardless, never a silent blank, and the report names the type
  so the plugin author can find their mistake. Plugin UI is build-time
  linked code — the same trust story 19 gave icons — so there is no
  sanitising ceremony around it; a plugin's own malformed UI is that
  plugin's declaration to fix.
...
time (vision scope); and no new editing gestures or canvas behaviour —
this story adds rendering and display, not interaction.
...
- A plugin declares node-type UI through the one `inventory` registration
  ...
  node, or type anywhere in it. (REQ-8, REQ-39, REQ-42, REQ-73)
...
- A node of a type with declared UI renders the plugin's component once
  ...
  and the live display state — status and per-port latest values, custom
  types in the form their serialiser produces — and owning the content
  between the ports; the ports remain editor-rendered, and wiring,
  unhooking, moving, selecting, deleting, panning, and zooming behave on
  it exactly as on any default node. Until the bundle arrives, and on
  every failure, the node renders by the default class — the editor stays
  usable, never blank. (REQ-39, REQ-34, REQ-38, REQ-47)
- Editing issued from inside a plugin's UI rides the existing operation
  ...
  on is rejected by 16's lock identically. A plugin's UI defines no
  second editing path and the definition gains no field it did not
  already carry. (REQ-2, REQ-25, REQ-41)
...
- The component contract is versioned and the version rides the listing
  facts; a bundle naming an unknown contract version, or failing to load,
  is reported through 18's surfaces naming the type, and the node falls
  back to the default rendering. Bundles load lazily — on the first node
  of the type or first display of the type's values — and a bundle that
...
- Focused tests cover: the registration path for both attachment points;
  ...
  unknown version; and a plugin-issued parameter edit applying through the
  ordinary operation path, including its rejection while a run is on. The
...
- The visible proof, run per DEVELOPMENT.md: open the editor example, now
  linking a plugin that ships a custom-UI node type and a custom type with
  a value renderer. Drop the custom node — it renders the plugin's own
  design, not the default class; wire a value of the plugin's custom type
  into it and watch the plugin's rendering of that value appear; wire the
  same type into a default utility node and see the value displayed at its
  output readout by the plugin's type-value component; edit a parameter
  through the plugin's own control and see it land in the sidebar field,
  in a second tab, and after a reload. Rename the node in the sidebar and
...
- 2026-09-16 — UX calls settled here so plugin authors and later stories
  inherit them rather than relitigate: ports stay editor-rendered on
  custom nodes — the gestures must not change because a node looks
  different, and the canvas library needs real ports to wire to; the
  sidebar stays generic rather than growing a plugin-authorable second
  surface — one consumer today does not justify a second UI contract; the
  serialisation is one-directional — nothing carries a stream value from
  the browser back, so a deserialiser would be a mechanism with no user;
  bundles load lazily so a plugin-rich palette stays fast; and there is
  no settings or toggle around custom UI — a plugin declares UI, the
  editor renders it, and the default rendering is the fallback, not an
  option to choose.
