...
placeholder stays inert. A right-click acts on the selection too, and
never disturbs it: a node already selected keeps the whole selection, an
unselected one is selected alone first as a plain click does, and the
context menu opens with the packaging gestures — package the selection
into a group, unpack its group instances. The sidebar with a multi-selection
...
The packaging itself is a gesture. A non-empty selection packages through
the context menu or Ctrl+G: a small dialog asks the group's name,
pre-filled with an available suggestion so Enter alone commits, Esc
cancelling quietly — and the server derives everything else from the
definition it holds. The edges crossing the selection's boundary become
the group's exposed ports, each taking the bound inner port's name (a
numbered name where two would collide) and declaring that inner port's
types; the packaged nodes' stored positions move into the body as offsets
from the selection's centroid; the collapsed instance sits at that
centroid and takes the selection, every outside wire that touched the
selection now landing on its ports. A name the document already uses, or
one a linked type already holds, is refused naming the clash, and a
boundary crossing an unknown-typed placeholder inside the selection is
refused naming the node — both leaving the definition untouched. Unpack
runs the same entries the other way, one operation per group instance in
the selection: the nodes reappear at their stored offsets from where the
instance sat, the wires re-attach through the bindings, a value typed on
an unconnected exposed input lands on the bound inner input as its
parameter, and the definition leaves with the last instance, kept while
others remain. Both are single atomic edits under the run lock, and a
packaged graph saves, reloads, and runs headless like any other.
