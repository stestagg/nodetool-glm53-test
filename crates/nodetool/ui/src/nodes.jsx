...
// node, because space is at a premium. An input a wire reaches shows its
// connected state. It works when a plugin provides nothing but the
// definition. A node whose type reference is missing from the listing
// renders as an inert placeholder instead — never silently dropped.

import { Handle, Position } from '@xyflow/react'

function Port({ port, kind, wired = false }) {
  const incoming = kind === 'in'
  return (
    <div
      className={`port ${kind}${wired ? ' connected' : ''}`}
...
export function TypeNode({ data, selected }) {
  const { node, type, wiredInputs } = data
  return (
